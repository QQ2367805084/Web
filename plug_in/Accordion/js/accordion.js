  ...
  var Accordion = function (obj,option){
    ...
    if(this.defaults.arrangement === "horizontal"){
      ...
      this.li.hover(function(e) {
        ...
        console.log(self.eachSmallImgWidth)
        $(this).addClass('active').css("width",1917);
        console.log(self.defaults.animateWidth)
        if(self.LessThanIE8){
    ...
    defaultDOM:function(){
      ...
      if(!imgArr) return;
        ...
        $.each( imgArr , function(index, val) {
          ...
          val.src = 'image/'+index+'.png';
          if(index<=4){
            liHtml += 
            '<li class="accordion-li">'+
              '<a href="'+ val.url +'">'+
                '<img class="accordion-img" src="'+ '../plug_in/Accordion/'+val.src +'" alt="'+ val.alt +'"/>'+
              '</a>'+
          ...
          // console.log("打印",'../plug_in/Accordion/'+val.src)
        });
