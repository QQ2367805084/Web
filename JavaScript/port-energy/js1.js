(function(window){const l=42,r=10,w=350,h=50,PI=Math.PI
const L=l+r*2
...
function draw(ctx,operation,x,y){ctx.beginPath()
ctx.moveTo(x,y)
ctx.lineTo(x+l/2,y)
...
class jigsaw{constructor(el,success,fail){this.el=el
...
this.slider.addEventListener('mousedown',function(e){originX=e.x,originY=e.y
isMouseDown=true})
...
if(spliced){if(TuringTest){addClass(this.sliderContainer,'sliderContainer_success')
this.success&&this.success()}else{addClass(this.sliderContainer,'sliderContainer_fail')
this.text.innerHTML='鍐嶈瘯涓€娆�'
this.reset()}}else{addClass(this.sliderContainer,'sliderContainer_fail')
this.fail&&this.fail()
...
reset(){this.sliderContainer.className='sliderContainer'
...
this.draw()}}
window.jigsaw={init:function(element,success,fail){new jigsaw(element,success,fail).init()}}}(window))